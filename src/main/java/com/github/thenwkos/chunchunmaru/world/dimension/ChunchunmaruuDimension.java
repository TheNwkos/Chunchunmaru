...
package com.github.thenwkos.chunchunmaru.world.dimension;
...
import com.github.thenwkos.chunchunmaru.item.ChunchunmaruuItem;
import com.github.thenwkos.chunchunmaru.block.ChunoreBlock;
import com.github.thenwkos.chunchunmaru.ChunchunmaruModElements;

@ChunchunmaruModElements.ModElement.Tag
public class ChunchunmaruuDimension extends ChunchunmaruModElements.ModElement {
	@ObjectHolder("chunchunmaru:chunchunmaruu_portal")
	public static final CustomPortalBlock portal = null;
	public ChunchunmaruuDimension(ChunchunmaruModElements instance) {
		super(instance, 3);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(ChunoreBlock.block).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("chunchunmaru:chunchunmaruu"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("chunchunmaruu_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("chunchunmaruu_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("chunchunmaruu_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new ChunchunmaruuItem().setRegistryName("chunchunmaruu"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("chunchunmaruu_portal");
		}
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation(("chunchunmaru:obsessed-with-you"))),
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("chunchunmaru:chunchunmaruu"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("chunchunmaru:chunchunmaruu")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == ChunoreBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? ChunoreBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, ChunoreBlock.block.getDefaultState(), 3);
					}
				}
			}
